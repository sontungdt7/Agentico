...
const MINER_PATH = process.env.MINER_PATH ??
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', (code) => {
            ...
            if (code === 0)
                ...
                reject(new Error(`Process exited ${code}: ${(stderr || stdout).slice(0, 500)}`));
        });
    ...
    try {
        ...
        if (currentBlock === undefined || feeSplitterNonce === undefined) {
            ...
            if (feeSplitterNonce === undefined) {
                feeSplitterNonce = Number(await client.getTransactionCount({
        ...
        if (body.currency)
            env.CURRENCY = body.currency;
        // 1. Run GetInitCodeHashSepolia
        const forgeOut = await exec('forge', [
            'script',
        ...
        // 2. Run address-miner
        const minerOut = await exec(MINER_PATH, [
            initCodeHash,
            HOOK_MASK,
            '-m',
            agenticoLauncher,
